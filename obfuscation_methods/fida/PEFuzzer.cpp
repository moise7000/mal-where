...
// Types Windows pour compatibilite cross-platform
#ifdef _WIN32
    #include <windows.h>
#else
...
// Macro offsetof pour GCC 4.4.7 si necessaire
#ifndef offsetof
...
private:
    ...
    bool loadFile(const std::string& path) {
        // GCC 4.4.7 ne supporte pas std::string dans ifstream
        ...
        std::cout << "[+] PE charge: " << peData.size() << " bytes" << std::endl;
        return true;
    }

    bool saveFile(const std::string& path) {
        // GCC 4.4.7 ne supporte pas std::string dans ofstream
        std::ofstream file(path.c_str(), std::ios::binary);
        if (!file) {
            std::cerr << "[!] Impossible d'ecrire: " << path << std::endl;
            return false;
        }
        file.write((char*)peData.data(), peData.size());
        file.close();
        std::cout << "[+] Sauvegarde: " << path << std::endl;
        return true;
    }
...
void printUsage() {
    std::cout << "\n";
    std::cout << "PE Header Fuzzer - Testeur de robustesse IDA\n";
    std::cout << "Usage: pe_fuzzer.exe <input.exe> <output.exe> [options]\n";
    std::cout << "\n";
    std::cout << "Options de corruption:\n";
    std::cout << "  -sections <n>        Modifier NumberOfSections (ex: -sections 999)\n";
    std::cout << "  -optsize <n>         Modifier SizeOfOptionalHeader (ex: -optsize 0)\n";
    std::cout << "  -codesize <hex>      Modifier SizeOfCode (ex: -codesize FFFFFFFF)\n";
    std::cout << "  -entrypoint <hex>    Modifier AddressOfEntryPoint (ex: -entrypoint DEADBEEF)\n";
    std::cout << "  -initdata <hex>      Modifier SizeOfInitializedData (ex: -initdata 99999999)\n";
    std::cout << "  -corrupt-pe          Corrompre la signature PE\n";
    std::cout << "  -corrupt-dos         Remplir le DOS stub de INT3\n";
    std::cout << "  -info                Afficher les infos sans modifier\n";
    std::cout << "\n";
    std::cout << "Exemples:\n";
    std::cout << "  pe_fuzzer.exe calc.exe calc_fuzzy.exe -sections 100\n";
    std::cout << "  pe_fuzzer.exe test.exe test_crash.exe -codesize FFFFFFFF -sections 50\n";
    std::cout << "  pe_fuzzer.exe app.exe app_info.exe -info\n";
    std::cout << std::endl;
}
...
int main(int argc, char* argv[]) {
    ...
    for (int i = 3; i < argc; i++) {
        ...
        if (arg == "-info") {
            ...
            fuzzer.corruptSizeOfCode(std::strtoul(argv[++i], NULL, 16));
            modified = true;
        }
        else if (arg == "-entrypoint" && i + 1 < argc) {
            fuzzer.corruptEntryPoint(std::strtoul(argv[++i], NULL, 16));
            modified = true;
        }
        else if (arg == "-initdata" && i + 1 < argc) {
            fuzzer.corruptSizeOfInitializedData(std::strtoul(argv[++i], NULL, 16));
            modified = true;
        }
    ...
    if (!infoOnly && modified) {
        ...
        std::cout << "\n[!] ATTENTION: Le fichier modifie peut crasher IDA!" << std::endl;
        std::cout << "   Testez avec prudence dans une VM ou un environnement isole." << std::endl;
    } else if (!modified && !infoOnly) {
        std::cout << "\n[i] Aucune modification specifiee. Utilisez -info pour voir les details." << std::endl;
    }
