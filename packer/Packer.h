...
public:
    ...
    * @param encoding Type d'encodage de la clé (PLAIN, BASEE64, BASE32)
    */
   ...
   */
    ...
    */
    std::string stub(const std::vector<unsigned char>& m, size_t originalSize) const;

    /**
    * Fonction qui déchiffre et décompresse un message (surcharge pour un string)
    * @param m Le message chiffré et compressé sous forme de vecteur de bytes
    * @param originalSize La taille originale du message avant compression
